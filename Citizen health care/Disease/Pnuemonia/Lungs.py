...
model = load_model(r"Disease\Pnuemonia\pneumonia_detection_ai_version_2.h5")

# Function to preprocess the image
def preprocess_image(image_path):
    img = PILImage.open(image_path).convert("L")  # Convert image to grayscale
    img = img.resize((200, 200))  # Resize image to (200, 200) for model input
...
class PneumoniaApp(MDApp):
    def build(self):
        ...
        self.title = "Pneumonia Disease Classification"
        ...
        self.heading_label = MDLabel(text="Pneumonia Classification", halign="center", theme_text_color="Secondary", font_style="H2", bold=True)
    ...
    def predict_image(self, *args):
        ...
        dialog_title = "Pneumonia Classification Result"
        dialog_text = (
            "The image is classified as Pneumonia."
            if result > 0.95
            else "The image is classified as Pnuemonia ."
        )
        self.success_dialog = MDDialog(
                title="Pneumonia Classification Result",
                text=dialog_text,
                size_hint=(0.7, 1),
                auto_dismiss=False,
                buttons=[
