...
model = load_model(r"Disease\malaria\malaria_mass.h5")

# Function to preprocess the image
def preprocess_image(image_path):
    #img = convert("L")  # Convert image to grayscale
    img = PILImage.open(image_path).resize((40,40))  # Resize image to (200, 200) for model input
...
class PneumoniaApp(MDApp):
    def build(self):
        ...
        self.title = "malaria Disease Classification"
        ...
        self.heading_label = MDLabel(text="Malaria Classification", halign="center", theme_text_color="Secondary", font_style="H2", bold=True)
    ...
    def predict_image(self, *args):
        ...
        dialog_title = "Malaria Classification Result"
        dialog_text = (
            "The image is classified as Normal."
            if result > 0.5
            else "The image is classified as having malaria ."
        )
